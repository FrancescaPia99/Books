//
//  BooksModel.swift
//  Books
//
//  Created by Francesca Pia De Rosa on 16/11/23.
//

import Foundation

class BooksModel {
    
    var books1 = [
    Book(cover: "1q84", title: "1Q84", author: "Haruki Murakami"),
    Book(cover: "it", title: "IT", author: "Steven King"),
    Book(cover: "doriangray", title: "The Portrait of Dorian Gray", author: "Oscar Wilde"),
    Book(cover: "lotr", title: "The Lord of the Rings", author: "JRR Tolkien"),
    Book(cover: "faust", title: "Faust", author: "Johann Wolfgang von Goethe"),
    Book(cover: "colera", title: "El amor en los tiempos del cólera", author: "Gabriel Garcia Marquez")
    ]
    var books2 = [
    Book(cover: "shadventures", title: "The Adventures of Sherlock Holmes", author: "Arthur Conan Doyle"),
    Book(cover: "shreturn", title: "The Return of Sherlock Holmes", author: "Arthur Conan Doyle"),
    Book(cover: "brownsuit", title: "The Man in the Brown Suit", author: "Agatha Christie"),
    Book(cover: "prince", title: "The Little Prince", author: "Antoine de Saint-Exupéry"),
    Book(cover: "murderonthelinks", title: "The Murder on the Links", author: "Agatha Christie"),
    Book(cover: "mesopotamia", title: "Murder in Mesopotamia", author: "Agatha Christie")
    ]
    var books3 = [
    Book(cover: "dance", title: "Dance", author: "Anonymous"),
    Book(cover: "love", title: "Love", author: "Anonymous"),
    Book(cover: "easteregg", title: "La libertà di amarsi", author: "Matthew Andrea D'Alessio \nFrancesca Pia De Rosa"),
    Book(cover: "easteregg2", title: "La libertà di sperare", author: "Matthew Andrea D'Alessio \nFrancesca Pia De Rosa"),
    Book(cover: "pony", title: "My Little Country", author: "The Supreme Leader")
    Book(cover: "realme", title: "Real Me", author: "Francesca Pia De Rosa")
    ]
    
}
